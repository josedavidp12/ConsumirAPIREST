...
<form action="{{ route('people.update', ['idPeople' => $people['id']]) }}" method="POST" class="w-50 mx-auto">
    ...
    <div class="form-group">
        ...
        <input type="text" name="direccion" id="direccion" value="{{ $people['direccion'] }}" class="form-control"
            required>
    </div>
    <div class="form-group">
        <label for="telefono">Teléfono</label>
        <input type="text" name="telefono" id="telefono" value="{{ $people['telefono'] }}" class="form-control"
            required>
    </div>
